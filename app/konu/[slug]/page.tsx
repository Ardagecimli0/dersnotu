  ...
  useEffect(() => {
    const fetchNote = async () => {
      try {
        ...
        const fetchedNote = await notesApi.getBySlug(slug);
        setNote(fetchedNote);
        ...
        const savedComments = localStorage.getItem(`comments_${fetchedNote.id}`);
        if (savedComments) {
          setComments(JSON.parse(savedComments));
        }

        const savedReactions = localStorage.getItem(`reactions_${fetchedNote.id}`);
        if (savedReactions) {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      localStorage.setItem(`reactions_${note.id}`, JSON.stringify({
                        counts: newReactions,
              ...
              <div className="flex justify-end">
                <Button 
                  onClick={() => {
                    ...
                    localStorage.setItem(`comments_${note.id}`, JSON.stringify(updatedComments));
